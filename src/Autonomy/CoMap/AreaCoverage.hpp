 ...
 * the segments.
 */
Math::Matrix intersectSegments(const Math::Matrix &seg1,
                               const Math::Matrix &seg2, bool *isInSegment) {
  ...
  if ((std::abs(b1 - b2) < 0.5) && (std::abs(m1 - m2) < 0.5))
    ...
    *isInSegment =
        (std::abs(xi - seg1(0, 0)) <= std::abs(seg1(0, 0) - seg1(0, 1))) &&
...
Math::Matrix intersectSegmentPolygon(const Math::Matrix &poly, double height) {
  ...
  double data[] = {min(easting_row) - 10, max(easting_row) + 10, height,
                   height};
  ...
  for (int i = 0; i < sides; i++) {
    if (i < sides - 1)
      intersection =
          intersectSegments(poly.get(0, 1, i, i + 1), segment, &in_poly);
    else {
      double coords[] = {poly(0, sides - 1), poly(0, 0), poly(1, sides - 1),
                         poly(1, 0)};
 ...
 * @return Math::Matrix
 */
Math::Matrix getAsMatrix(const std::vector<std::pair<double, double>> &coords) {
  double tmp[] = {0, 0};
...
Math::Matrix getSurveyPath(const Math::Matrix &poly, double direction,
                           double width, double diam) {
  ...
  double data[] = {std::cos(-direction), -std::sin(-direction),
                   std::sin(-direction), std::cos(-direction)};
  ...
  for (int i = 1; i < n_rows; i++)
    rows = rows.horzCat(intersectSegmentPolygon(
        rotated_poly, i * row_width + lower_north + offset));

  double data2[] = {std::cos(direction), -std::sin(direction),
                    std::sin(direction), std::cos(direction)};
 ...
 */
void calculateSurveyDirection(const Math::Matrix &poly, double &diameter,
                              double &ang) {
  ...
  for (double angle = 0.0; angle < (Math::c_pi * 1.1); angle += step) {
    double data[] = {std::cos(-angle), -std::sin(-angle), std::sin(-angle),
                     std::cos(-angle)};
  ...
  double data[] = {std::cos(-angle_min), -std::sin(-angle_min),
                   std::sin(-angle_min), std::cos(-angle_min)};
  rot_mtx = Matrix(data, 2, 2);
  rotated_poly = rot_mtx.multiply(poly);
  northing_row = rotated_poly.row(1);
 ...
 */
void calculateSurveyDirection(
    const std::vector<std::pair<double, double>> &coords, double &diameter,
    double &angle) {
...
void getCoveragePath(std::vector<std::pair<double, double>> &area,
  ...
  double data2[] = {std::cos(angle), -std::sin(angle), std::sin(angle),
                    std::cos(angle)};
  rotation = Matrix(data2, 2, 2);
  rows = rotation.multiply(rows);
}
