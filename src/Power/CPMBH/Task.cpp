...
namespace Power
{
  namespace CPMBH
  {
    //! Device driver for CPMB-H (Copter Power Management Board) for Hercules platform.
    //!
    ...
    static const unsigned c_pwrs_count = 6;
    //! Number of ADC channels.
    static const unsigned c_adcs_count = 4;
    ...
    struct Task: public DUNE::Tasks::Task
    {
      ...
      getMonitors(void)
      {
        ...
        if (!m_ctl->sendFrame(frame))
          throw std::runtime_error(DTR("Failed to send frame to CPMB-H. Check connection!"));
