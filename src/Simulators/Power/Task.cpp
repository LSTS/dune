...
namespace Simulators
{
  ...
  namespace Power
  {
    ...
    struct Task: public Tasks::Task
    {
      ...
      IMC::PowerChannelState m_power_channels[c_max_power_channel];
      ...
      onEntityReservation(void)
      {
        ...
        else
        {
          ...
          for (int i = 0; i < (int)m_args.power_channel_names.size(); ++i)
          {
            ...
            m_power_channels[i].setSourceEntity(eid);
            m_power_channels[i].name = m_args.power_channel_names[i];
            m_power_channels[i].state = IMC::PowerChannelState::PCS_ON;
            inf("Power channel entity '%s' reserved with id %u.", m_power_channels[i].name.c_str(),
                eid);
            // Voltage message
      ...
      consume(const IMC::PowerChannelControl* msg)
      {
        for (int i = 0; i < (int)m_args.power_channel_names.size(); ++i)
        {
          if (msg->name == m_power_channels[i].name)
          {
            switch (msg->op)
            {
              case IMC::PowerChannelControl::PCC_OP_TURN_OFF:
                m_power_channels[i].state = IMC::PowerChannelState::PCS_OFF;
                inf("Power channel '%s' turned OFF.", m_power_channels[i].name.c_str());
                break;
              case IMC::PowerChannelControl::PCC_OP_TURN_ON:
                m_power_channels[i].state = IMC::PowerChannelState::PCS_ON;
                inf("Power channel '%s' turned ON.", m_power_channels[i].name.c_str());
                break;
              case IMC::PowerChannelControl::PCC_OP_TOGGLE:
                if (m_power_channels[i].state == IMC::PowerChannelState::PCS_ON)
                {
                  m_power_channels[i].state = IMC::PowerChannelState::PCS_OFF;
                  inf("Power channel '%s' toggled to OFF.", m_power_channels[i].name.c_str());
                }
                else
                {
                  m_power_channels[i].state = IMC::PowerChannelState::PCS_ON;
                  inf("Power channel '%s' toggled to ON.", m_power_channels[i].name.c_str());
                }
                break;
              default:
          ...
          if (m_delay_timer.overflow())
          {
            ...
            if (!m_no_power_channel)
            {
              for (int i = 0; i < (int)m_args.power_channel_names.size(); ++i)
              {
                ...
                if (m_power_channels[i].state == IMC::PowerChannelState::PCS_OFF)
                {
