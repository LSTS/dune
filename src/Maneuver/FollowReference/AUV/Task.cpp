...
struct Task: public DUNE::Maneuvers::Maneuver {
	...
	Task(const std::string& name, Tasks::Context& ctx) :
		...
		param("Vehicle Type", m_args.vehicle_type).defaultValue("AUV").description(
		...
		param("Default Speed", m_args.default_speed).defaultValue("50").description(
				"Speed to use in case no speed is given by reference source.");

		param("Default Speed Units", m_args.default_speed_units).defaultValue(
				"%").description(
				"Units to use for default speed (one of 'm/s', 'rpm' or '%').");

		param("Default Z", m_args.default_z).defaultValue("0").units(
				Units::Meter).description(
				"Default z when no vertical reference is given.");

		param("Default Z Units", m_args.default_z_units).defaultValue("DEPTH").units(
				Units::Meter).description(
				"Units to use for default z reference (one of 'DEPTH', 'ALTITUDE' or 'HEIGHT')");
	...
	uint8_t pathDifferences(const IMC::DesiredPath *msg1,
		...
		if (msg1->end_z != msg2->end_z
				|| msg1->end_z_units != msg2->end_z_units)
			flags |= Z_CHANGED;

		if (msg1->speed != msg2->speed
				|| msg1->speed_units != msg2->speed_units)
			flags |= SPEED_CHANGED;
	...
	void guide(const IMC::PathControlState* pcs, IMC::Reference* ref,
		...
		double xy_dist = WGS84::distance(desired_path.end_lat,
				desired_path.end_lon, 0, curlat, curlon, 0);
		bool at_z_target = z_dist < m_args.vertical_tolerance;
		bool at_xy_target = xy_dist
				< std::fabs(ref->radius) + m_args.horizontal_tolerance;
		...
		if (m_fref_state.state == IMC::FollowRefState::FR_LOITER
				|| m_fref_state.state == IMC::FollowRefState::FR_ELEVATOR) {
		...
		if (at_xy_target
				|| (m_args.vehicle_type.compare("UAV") == 0 && near_ref))
...
private:
	...
	void updateSpeed(const IMC::Reference* ref,
		...
		if ((ref->flags & IMC::Reference::FLAG_SPEED)
			...
			desired_path.speed_units = parseSpeedUnitsStr(
					m_args.default_speed_units);
		}
	}
	...
	double updateEndZ(const IMC::Reference* ref,
			IMC::DesiredPath& desired_path) {
		...
		case (IMC::Z_HEIGHT):
			z_dist = std::abs(
					desired_path.end_z - (m_estate.height - m_estate.z));
			break;
		default:
			z_dist = 0;
			break;
	...
	void dispatchDesiredPath(IMC::DesiredPath desired_path) {
		...
		if (changedRadius) {
			inf(DTR("Loiter radius reference changed to %f"),
					desired_path.lradius);
		}
