                    ...
                    while(getline(file, line))  // tant que l'on peut mettre la ligne dans "line"
                    {
                            ...
                            if( line.substr (0,3) == "IMG"){
                                    ...
                                    cv::Mat A = cv::imread(path,CV_LOAD_IMAGE_GRAYSCALE);
                                     if ( A.data != NULL){//If the Image exists in the given path
                                                    ...
                                                    Liste_Images.push_back(Image(Name, A,Translation,Rotation,IntrinsicMatrix,Radial_distortion,Tangential_distortion));
