...
namespace MiniASV
{
  ...
  struct Task : public DUNE::Tasks::Task
  {
    ...
    {
      if (msg->getDestination() == 42)
      {
        double aux = msg->getTimeStamp();
        ...
        if (aux != m_tstamp)
        {
          ...
          if (msg->id == 1)
            m_driver->m_miniASVData.pwmR = msg->duty_cycle;
          else if (msg->id == 2)
            m_driver->m_miniASVData.pwmL = msg->duty_cycle;
        }
      }
    }
    ...
    onMain(void)
    {
      ...
      while (!stopping())
      {
        ...
        /* if (dir == 0)
        {
          send = String::str("@PWM,L,%d,*", 25);
          m_driver->sendCommandNoRsp(send.c_str());

          send = String::str("@PWM,R,%d,*", 25);
          m_driver->sendCommandNoRsp(send.c_str());
        }
        else
        {
