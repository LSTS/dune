...
// Author: José Braga                                                       *
// Author: Maria Costa (Adapted Control.ASV.HeadingAndSpeed to add Sway).   *
//***************************************************************************
  ...
  namespace ROV
  {
    namespace HorizontalPlaneV2
    {
      ...
      struct Task: public Tasks::Task
      {
        ...
        IMC::SetThrusterActuation m_act[3];
        //! Last motor actuation.
        IMC::SetThrusterActuation m_last_act[3];
        //! Motor rpm
        IMC::Rpm m_rpm[3];
        //! Estimated State.
        ...
        bool m_common;
        //! Apply sway actuation.
        bool m_sway;
        //! Motor rpm entity id.
        uint16_t m_rpm_eid[3];
        ...
        Task(const std::string& name, Tasks::Context& ctx):
          ...
          m_scope_ref(0)
          {
            ...
            param("Enable Sway", m_args.sway)
            .defaultValue("false")
            .description("Enable use of sway.");
            ...
            param("Entity Label - Sway Motor", m_args.eid_sway)
            .defaultValue("Motor - Sway")
            .description("Entity label of sway motor rpm");
          ...
          onEntityResolution(void)
          {
            ...
            try
            {
              m_rpm_eid[2] = resolveEntity(m_args.eid_sway);
            }
            catch (...)
            {
              m_rpm_eid[1] = 0xffff;
          ...
          consume(const IMC::EstimatedState* msg)
          {
            ...
            // Thrust forward.
            if (thrustForward(err_yaw) || m_sway)
            {
              // Velocity controller.
            ...
            dispatchThrust(m_act[2].value, tstep, 2);
          }
          ...
          consume(const IMC::Rpm* msg)
          {
            ...
            if (msg->getSourceEntity() == m_rpm_eid[2])
              m_rpm[2].value = msg->value;
          }
          ...
          thrustForward(float yaw_err)
          {
            ...
            else
            {
              ...
              if (m_args.sway && (std::fabs(yaw_err) > Angles::radians(89))
                              && (std::fabs(yaw_err) < Angles::radians(91)))
              {
                m_sway = true;
                debug("Sway!");
                m_dyaw_sway = m_estate.psi;
              }
            }
