...
#ifndef DUNE_COLLISION_AVOIDANCE_AUTONAUTCAS_SB_MPC_HPP_INCLUDED_
#define DUNE_COLLISION_AVOIDANCE_AUTONAUTCAS_SB_MPC_HPP_INCLUDED_
  ...
  namespace CollisionAvoidance
  {
	namespace AutonautCAS
	{
	  ...
	  public:
		...
		void
		getBestControlOffset(double& u_os_best, double& psi_os_best, double u_d, double psi_d_,
			                 const std::vector<double>& asv_state, const Eigen::Matrix<double, -1, 2>& waypoints_,
							 const Math::Matrix& obst_states, Math::Matrix static_obst_states_, double &cost_mra, obstacle*& obs)
		{
		  double cost = INFINITY;
		  double cost_k = 0, cost_i = 0, cost_o = 0; // cost_ac = 0;
		  int n_obst, n_samp = T_ / DT_, n_samp_pred = n_samp / pred_step;
		  ...
		  // 	double Chi_ca_i = 0.0;
		  // 	int i_return_to_path = 0, ik_return_to_path = 0; // iter at which ASV can return to path
		  ...
		  /* else */ if (obst_states.rows() > 0)
		  {
			...
			if (OBST_FILTER_ON_)
			{
			  for (size_t j = 0; j < oldObstacles_.size(); j++)
			  {
				...
				if (oldObstacles_[j]->durationTracked >= T_TRACKED_LIMIT_ &&
				    (oldObstacles_[j]->durationLost < T_LOST_LIMIT_ || oldObstacles_[j]->P(0, 0) <= 5.0 /* 2.0 */))
				{
				  // update object in old list and move it to the new list,
			...
			v_s(0) = asv_state[3];
			v_s(1) = asv_state[4];
			rot2d(Math::Angles::normalizeRadian(asv_state[2]), v_s);
			...
			double psi_path = atan2(waypoints(1, 1) - waypoints(0, 1), waypoints(1, 0) - waypoints(0, 0)); // path course
			// distance of track from wp0 to wp1
			double track_dist = std::sqrt(std::pow(waypoints(1, 0) - waypoints(0, 0), 2) + std::pow(waypoints(1, 1) - waypoints(0, 1), 2));
			...
			double along_track_dist = fabs((0 - asv_state[0]) * std::cos(psi_path) + (300 - asv_state[1]) * std::sin(psi_path));
			if (track_dist > R + asv->getL()) // else last WP reached, assume constant course!
			{
			  // asv-wp radius
			  double asv_wp_radius_sqrd = std::pow(asv_state[0] - waypoints(1, 0), 2) + std::pow(asv_state[1] - waypoints(1, 1), 2);
			  // along-track error using Eq (10.58 or 10.10) in Fossen 2011
			  along_track_dist = fabs((waypoints(1, 0) - asv_state[0]) * std::cos(psi_path) + (waypoints(1, 1) - asv_state[1]) * std::sin(psi_path));
			...
			for (int k = 0; k < n_obst; k++)
			{
			  ...
			  phi_0 = PHI_0(k) - Math::Angles::normalizeRadian(asv_state[2]);
			  phi_0 = Math::Angles::normalizeRadian(phi_0);
			  // obstacle at starboard/port side
			  if (obst_vect[k]->SB_0 > -1 &&
				 ((phi_0 > Math::Angles::radians(-15) && phi_0 < Math::Angles::radians(15)) ||
				  (phi_0 < Math::Angles::radians(-165) && phi_0 >= Math::Angles::radians(-180)) ||
				  (phi_0 > Math::Angles::radians(165) && phi_0 <= Math::Angles::radians(180))))
			  ...
			  double los_phi = std::acos(v_s.dot(los_0) / v_s.norm());
			  if (obst_vect[k]->AH_0 > -1 && los_phi > Math::Angles::radians(PHI_AH_) && los_phi < Math::Angles::radians(90.0)) // 112.5?
				AH_0(k) = obst_vect[k]->AH_0;
			  else
				AH_0(k) = v_s.dot(los_0) > std::cos(Math::Angles::radians(PHI_AH_)) * v_s.norm(); // 90.0 or PHI_AH_?

			  obst_vect[k]->AH_0 = (int)AH_0(k);
			  // asv overtaking obstacle
			  OTG_0(k) = (v_s.dot(v_o) > std::cos(Math::Angles::radians(PHI_OT_)) * v_s.norm() * v_o.norm()
			  ...
			  if (guidance_strategy < 2) // check angle made with LOS to next WP
				OTG_0(k) = OTG_0(k) && v_o.dot(los_s_wp1) > std::cos(Math::Angles::radians(45)) * v_o.norm(); // PHI_OT_?

			  // asv being overtaken by obstacle
			  OT_0(k) = v_s.dot(v_o) > std::cos(Math::Angles::radians(PHI_OT_)) * v_s.norm() * v_o.norm() && v_s.norm() < v_o.norm() && !AH_0(k) && dist_0 < D_CLOSE_;

			  if (guidance_strategy < 2) // check angle made with LOS to next WP
				OT_0(k) = OT_0(k) && v_o.dot(los_s_wp1) > std::cos(Math::Angles::radians(PHI_OT_)) * v_o.norm();

			  // Obstacle out of way? use it to constrain CRG scenario only?
			  OBS_PASSED_0(k) = ((v_o.dot(-los_0) < std::cos(Math::Angles::radians(112.5)) * v_o.norm() /* obstacle's perspective */ && !OTG_0(k))
			  					|| (v_s.dot(los_0) < std::cos(Math::Angles::radians(112.5)) * v_s.norm() /* ASV's perspective */ && !OT_0(k)))
								&& dist_0 > D_SAFE_; // D_SAFE_ or D_CLOSE_?

			  if (guidance_strategy < 2 && dist_0 > D_CLOSE_)
				OBS_PASSED_0(k) = OBS_PASSED_0(k) && v_s.dot(los_s_wp1) > std::cos(Math::Angles::radians(PHI_HO_)) * v_o.norm();

			  // Crossing in progress
			  CRG_0(k) = v_s.dot(v_o) < std::cos(Math::Angles::radians(PHI_CR_)) * v_s.norm() * v_o.norm() && !OBS_PASSED_0(k) // && AH_0(k)  test OBS_PASSED_0(k)
						 && !OTG_0(k) && !OT_0(k)
						 && v_o.norm() > 0.25; // 0.05?

			  if (guidance_strategy < 2 && dist_0 > D_CLOSE_)// check angle made with LOS to next WP
				CRG_0(k) = CRG_0(k) && v_o.dot(los_s_wp1) < std::cos(Math::Angles::radians(PHI_CR_)) * v_o.norm();

			  // Head-on transition in progress
			  HOT_0(k) = v_o.norm() > 0.25 // 0.05?
						 && v_s.dot(v_o) < -std::cos(Math::Angles::radians(PHI_HO_)) * v_s.norm() * v_o.norm() // 89.5
						 && v_s.dot(los_0) > std::cos(Math::Angles::radians(PHI_AH_)) * v_s.norm() // reduces scope!
						 && AH_0(k);

			  if (guidance_strategy < 2)
			    HOT_0(k) = HOT_0(k) && v_o.dot(los_s_wp1) < -std::cos(Math::Angles::radians(PHI_HO_)) * v_o.norm();
			}
			...
			{ // iterates through speed 'offsets', only 1 speed
			  ...
			  { // iterates through course offsets
				...
				{
				  // setup for guidance behavior simulation
				  if (cp_ > 0 && (0.0 == Chi_ca_[i] || (guidance_strategy < 2 && std::fabs(Chi_ca_[i]) >= Math::Angles::radians(90.0))))
					break;
				  ...
				  {
					if (Chi_ca_[i] > 0)
						Chi_ca_i = Chi_ca_[i] + Math::Angles::radians(15.0) * cp;
				  ...
				  if (std::fabs(Chi_ca_i) > Math::Angles::radians(90.0))
					break;

				  asv->linearPrediction(asv_state, u_d * P_ca_[j], psi_d + Chi_ca_i, waypoints, Chi_ca_i, cp,
										guidance_strategy, WP_R_, LOS_LA_DIST_, LOS_KI_);
				  // Compute worst cost associated with the current control behavior and the corresponding scenarios for each obsbtacle
				  cost_i = -1;
				  cost_o = 0;
				  ik_return_to_path = 0;
				  ...
				  for (int k = 0; k < n_obst; k++)
				  {
					...
					for (int l = 0; l < n_obst_branches; l++)
					{
					  ...
					  cost_k = costFunction(P_ca_[j], Chi_ca_[i], k, (bool)SB_0(k), (bool)CRG_0(k),
					                        (bool)OTG_0(k), (bool)OT_0(k), (bool)HOT_0(k), DIST_0(k),
											u_d, l, ik_return_to_path, obst_states, static_obst_states_, i);
		...
		inline void
		setPhiAH(double phi_AH)
		{
		  PHI_AH_ = phi_AH;
		}

		inline void
		setPhiOT(double phi_OT)
		{
		  PHI_OT_ = phi_OT;
		}

		inline void
		setPhiHO(double phi_HO)
		{
		  PHI_HO_ = phi_HO;
		}

		inline void
		setPhiCR(double phi_CR)
		{
		  PHI_CR_ = phi_CR;
		}
		...
		inline void
		setGuidanceStrategy(int guidance_strategy)
		{
		  GUIDANCE_STRATEGY_ = guidance_strategy;
		}

		inline void
		setWpR(double wp_radius)
		{
		  WP_R_ = wp_radius;
		}

		inline void
		setLosLaDist(double los_la_d)
		{
		  LOS_LA_DIST_ = los_la_d;
		}

		inline void
		setLosKi(double los_ki)
		{
		  LOS_KI_ = los_ki;
		}

		inline void
		setAngRange(double ang_range)
		{
		  ang_range_ = ang_range;
		}
	  ...
	  private:
		...
		double costFunction(double P_ca, double Chi_ca, int k, bool SB_0, bool CRG_0, bool OTG_0, bool OT_0, bool HOT_0, double DIST_0, double u_d, int l, int &ik_return_to_path, Math::Matrix obst_state, Math::Matrix static_obst_state, int chi_ca_index)
		{
		  ...
		  for (int i = 0; i < n_samp - 1; i += pred_step) // using 5s or 10s fixed intervals for DT_=0.5
		  {
			...
			//
			// 	 R_ground = (1 / std::pow(std::fabs(t - t0), P_G_)) * std::pow(risk_land, Q_);
			...
			if (obst_state.rows() > 0)
			{
			  ...
			  if (dist < d_close)
			  {
				...
				psi_o = Math::Angles::normalizeRadian(psi_o); // already normalized!
				phi = atan2(d(1), d(0)) - asv->m_psi[i];
				phi = Math::Angles::normalizeRadian(phi);
				psi_rel = psi_o - asv->m_psi[i];
				psi_rel = Math::Angles::normalizeRadian(psi_rel); // change to angle_diff?
				...
				if (phi < Math::Angles::radians(PHI_AH_))// obst ahead
				  d_safe_i = d_safe + asv->getL() / 2;
				else if (phi > Math::Angles::radians(PHI_OT_)) // obst behind
				  d_safe_i = 0.5 * d_safe + asv->getL() / 2;
				else
				  d_safe_i = d_safe + asv->getW() / 2;

				phi_o = atan2(-d(1), -d(0)) - obst_vect[k]->psi_;
				phi_o = Math::Angles::normalizeRadian(phi_o);

				if (phi_o < Math::Angles::radians(PHI_AH_)) // ship ahead
				  d_safe_i += d_safe + obst_vect[k]->getL() / 2; // d_safe +
				else if (phi_o > Math::Angles::radians(PHI_OT_))	// ship behind
				  d_safe_i += 0.5 * d_safe + obst_vect[k]->getL() / 2; // 0.5*d_safe +
				else
				  d_safe_i += d_safe + obst_vect[k]->getW() / 2; // d_safe +

				if (v_s.dot(v_o) > std::cos(Math::Angles::radians(PHI_OT_)) * v_s.norm() * v_o.norm() && v_s.norm() > v_o.norm())
				  d_safe_i = d_safe + asv->getL() / 2 + obst_vect[k]->getL() / 2;

				// Overtaken by obstacle
				OT = v_s.dot(v_o) > std::cos(Math::Angles::radians(PHI_OT_)) * v_s.norm() * v_o.norm() && v_s.norm() < v_o.norm();
				// Obstacle on starboard side
				SB = phi >= 0; // ENU: < 0, NED (MR): >= 0

				// Obstacle Head-on
				HO = v_o.norm() > 0.25 // 0.05?
					 && v_s.dot(v_o) < -std::cos(Math::Angles::radians(PHI_HO_)) * v_s.norm() * v_o.norm() && v_s.dot(los) > std::cos(Math::Angles::radians(PHI_AH_)) * v_s.norm();

				// Crossing situation: obstacle crossed!
				CR = v_o.norm() > 0.25 // 0.05?
				     && v_s.dot(v_o) < std::cos(Math::Angles::radians(PHI_CR_)) * v_s.norm() * v_o.norm() && v_s.dot(los) > std::cos(Math::Angles::radians(112.5)) * v_s.norm();
				...
				//   gCost = (1/std::pow(std::fabs(t-t0),P_))*std::pow(D_SAFE_/d_geo,Q_);
				// }
				...
				{ // changed from < to <=. dist is the predicted distance to obst, while DIST_0 is the current distance to obst
				  ...
				  R = (1 / std::pow(std::fabs(t - t0), P_)) * std::pow(d_safe_i / dist, Q_);
				  k_coll = K_COLL_ * asv->getL() * obst_vect[k]->getL(); // L-influence!
				  // k_coll = K_COLL_; // test: fixed penalty!
				  C = k_coll * std::pow((v_s - v_o).norm(), 2); // relative velocity
				  if (DIST_0 < d_safe_i)
				  {
					R = (1 / std::pow(fabs(t - t0), P_)) * std::pow(d_safe_i / dist, Q_ * 10);
					...
					{
					  C1 = k_coll * (1.0 / std::pow(v_s.norm() - (v_o.norm() + s_0), 2)); // speed
					  C2 = 100 * k_coll * (1.0 / std::pow(1.0 + Math::Angles::degrees(psi_rel), 2)); // relative course
					  C = C1 + C2;
					}
		  ...
		  H2 = K_P_ * std::pow(1 - P_ca, 2) + sqrChi(Chi_ca, k_chi_p, k_chi_sb) + deltaP(P_ca) + deltaChi(Chi_ca, k_dchi_p, k_dchi_sb);
		  // H2 = K_P_*(1-P_ca) + K_CHI_*std::pow(Chi_ca,2) + deltaP(P_ca) + deltaChi(Chi_ca);
		  cost = H1 + H2;
		  return cost;
		}
		...
		deltaP(double P_ca)
		{
			return K_DP_ * std::pow(P_ca_last_ - P_ca, 2);
		}

		double
		deltaChi(double Chi_ca, double k_dchi_p, double k_dchi_sb)
		{
		  double dChi = Chi_ca - Chi_ca_last_;
		  if (dChi < 0) // ENU: > 0, NED (MR): < 0					
			return k_dchi_p * std::pow(dChi, 2); // K_DCHI_P_
		  else if (dChi > 0) // ENU: < 0, NED (MR): > 0
			return k_dchi_sb * std::pow(dChi, 2); // _SB_
	  	  else
			return 0;
		}

		double
		sqrChi(double Chi_ca, double k_chi_p, double k_chi_sb)
		{
		  double dChi = Chi_ca - Chi_ca_last_;
		  if (dChi < 0) // ENU: > 0, NED (MR): < 0
			return k_chi_p * std::pow(Chi_ca, 2); // _P_
		  else
			return k_chi_sb * std::pow(Chi_ca, 2); // _SB_
		}

		void
		rot2d(double yaw, Eigen::Vector2d &res)
		{
		  Eigen::Matrix2d R;
		 ...
		 */
		double
		dist2staticObst(Eigen::Vector2d p1, Eigen::Vector2d v1, Eigen::Vector2d v2)
		{
		 ...
		 */
		int
		orientation(Eigen::Vector2d p, Eigen::Vector2d q, Eigen::Vector2d r)
		{
		 ...
		 */
		double
		distPoint2line(Eigen::Vector2d p1, Eigen::Vector2d q1, Eigen::Vector2d q2)
		{
		  Eigen::Vector3d a;
		  Eigen::Vector3d b;
		  a << (q1 - q2), 0;
		 ...
		 */
		bool
		isBehind(Eigen::Vector2d p1, Eigen::Vector2d v1, Eigen::Vector2d v2, double dist2Line)
		{
		  Eigen::Vector2d n, n1;
		  n = v2 - v1;
		  n1 << -n[1], n[0];
