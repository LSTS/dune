...
#ifndef DUNE_COLLISION_AVOIDANCE_AUTONAUTCAS_AUTONAUT_HPP_INCLUDED_
#define DUNE_COLLISION_AVOIDANCE_AUTONAUTCAS_AUTONAUT_HPP_INCLUDED_
  ...
  namespace CollisionAvoidance
  {
    namespace AutonautCAS
    {
      ...
      public:
        ...
        void
        linearPrediction(const std::vector<double> &state, double u_d, double psi_d,
                         const Eigen::Matrix<double, -1, 2> &waypoints_, double Chi_ca, int course_change_point,
                         int guidance_strategy, double R, double de, double Ki)
        {
          ...
          if (init_samp == 0)
          {
            m_psi(0) = -Math::Angles::normalizeRadian(state[2]); // bug fix: use normalized psi // MR interface output sign change
            psi_d = Math::Angles::normalizeRadian(psi_d); // bug fix: use normalized psi_d
          ...
          if (init_samp > 0 && guidance_strategy < 2)
          {
            ...
            bool leg_passed = los_wp0_wp1.dot(-los_s_wp1) > std::cos(Math::Angles::radians(90.0));
          ...
          for (int i = init_samp; i < m_n_samp - 1; i++)
          {
            ...
            psi_d = Math::Angles::normalizeRadian(psi_d);
        ...
        {
          m_psi(0) = Math::Angles::normalizeRadian(psi_d);
          m_x(0) = state[0] + m_os_x * std::cos(state[2]) - m_os_y * std::sin(state[2]);
          m_y(0) = state[1] + m_os_x * std::sin(state[2]) + m_os_y * std::cos(state[2]);
          m_u(0) = state[3];
          m_v(0) = state[4];
          m_r(0) = state[5];
          double r11, r12, r21, r22;
