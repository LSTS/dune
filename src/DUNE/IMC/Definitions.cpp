...
namespace DUNE
{
  namespace IMC
  {
    ...
    StartOnManeuver::StartOnManeuver(void)
    {
      m_header.mgid = 565;
      clear();
    }
    ...
    void
    StartOnManeuver::clear(void)
    {
      man_id.clear();
    }

    bool
    StartOnManeuver::fieldsEqual(const Message& msg__) const
    {
      const IMC::StartOnManeuver& other__ = static_cast<const StartOnManeuver&>(msg__);
      if (man_id != other__.man_id) return false;
      return true;
    }

    int
    StartOnManeuver::validate(void) const
    {
      return true;
    }

    uint8_t*
    StartOnManeuver::serializeFields(uint8_t* bfr__) const
    {
      uint8_t* ptr__ = bfr__;
      ptr__ += IMC::serialize(man_id, ptr__);
      return ptr__;
    ...
    uint16_t
    StartOnManeuver::deserializeFields(const uint8_t* bfr__, uint16_t size__)
    {
      const uint8_t* start__ = bfr__;
      bfr__ += IMC::deserialize(man_id, bfr__, size__);
      return bfr__ - start__;
    ...
    uint16_t
    StartOnManeuver::reverseDeserializeFields(const uint8_t* bfr__, uint16_t size__)
    {
      const uint8_t* start__ = bfr__;
      bfr__ += IMC::reverseDeserialize(man_id, bfr__, size__);
      return bfr__ - start__;
    ...
    void
    StartOnManeuver::fieldsToJSON(std::ostream& os__, unsigned nindent__) const
    {
      IMC::toJSON(os__, "man_id", man_id, nindent__);
    }
