...
#include <DUNE/Math/OldMatrix.hpp>
...
namespace DUNE
{
  namespace Math
  {
    ...
    double OldMatrix::precision = 1e-10;

    OldMatrix::OldMatrix(void):
      m_nrows(0),
      m_ncols(0),
      m_size(0),
      m_data(NULL),
    ...
    OldMatrix::OldMatrix(size_t r, size_t c)
    {
      if (!r || !c)
    ...
    OldMatrix::OldMatrix(size_t r, size_t c, double value)
    {
      if (!r || !c)
        throw Error("Invalid dimension!");
    ...
    OldMatrix::OldMatrix(const OldMatrix& m)
    {
      m_nrows = m.m_nrows;
      m_ncols = m.m_ncols;
      m_size = m.m_size;
    ...
    OldMatrix::OldMatrix(const double* data, size_t r, size_t c)
    {
      if (!r || !c)
        throw Error("Invalid dimension!");
    ...
    OldMatrix::OldMatrix(const double* diag, size_t n)
    {
      m_nrows = n;
      m_ncols = n;
      m_size = n * n;
    ...
    const double*
    OldMatrix::begin(void) const
    {
      return m_data;
    }

    const double*
    OldMatrix::end(void) const
    {
      return m_data + m_size;
    }

    const double*
    OldMatrix::cbegin(void) const
    {
      return m_data;
    }

    const double*
    OldMatrix::cend(void) const
    {
      return m_data + m_size;
    }

    int
    OldMatrix::rows(void) const
    {
      return m_nrows;
    }

    int
    OldMatrix::columns(void) const
    {
      return m_ncols;
    }

    int
    OldMatrix::size(void) const
    {
      return m_size;
    }

    bool
    OldMatrix::isSquare(void) const
    {
      return m_nrows == m_ncols;
    }

    bool
    OldMatrix::isVector(void) const
    {
      return m_nrows == 1 || m_ncols == 1;
    }

    bool
    OldMatrix::isRowVector(void) const
    {
      return m_nrows == 1;
    }

    bool
    OldMatrix::isRowVector(size_t c) const
    {
      return m_nrows == 1 && m_ncols == c;
    }

    bool
    OldMatrix::isColumnVector(void) const
    {
      return m_ncols == 1;
    }

    bool
    OldMatrix::isColumnVector(size_t r) const
    {
      return m_ncols == 1 && m_nrows == r;
    }

    bool
    OldMatrix::isEmpty(void) const
    {
      return m_size == 0;
    }

    void
    OldMatrix::fill(size_t r, size_t c, const double* data)
    {
      erase();
      resize(r, c);
      std::memcpy(m_data, data, m_size * sizeof(double));
    ...
    void
    OldMatrix::identity(void)
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");

      if (m_nrows != m_ncols)
        throw Error("OldMatrix is not square!");
    ...
    void
    OldMatrix::maxLimitValues(double max)
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    void
    OldMatrix::minLimitValues(double min)
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    void
    OldMatrix::trimValues(double min, double max)
    {
      maxLimitValues(max);
      minLimitValues(min);
    }
    ...
    void
    OldMatrix::trimValues(double lim)
    {
      maxLimitValues(lim);
      minLimitValues(- lim);
    }
    ...
    OldMatrix
    OldMatrix::get(size_t i1, size_t i2, size_t j1, size_t j2) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    OldMatrix&
    OldMatrix::set(size_t i1, size_t i2, size_t j1, size_t j2, const OldMatrix& m)
    {
    ...
    OldMatrix&
    OldMatrix::blkDiag(const OldMatrix& mx_in)
    {
      OldMatrix old = *this;
      OldMatrix mx_in_ = mx_in;
    ...
    OldMatrix&
    OldMatrix::vertCat(const OldMatrix& mx_in)
    {
    ...
    OldMatrix&
    OldMatrix::horzCat(const OldMatrix& mx_in)
    {
      if (m_nrows != mx_in.m_nrows && !isEmpty())
        throw Error("Invalid index!");

      OldMatrix old = *this; //  <=> OldMatrix old(*this);
      OldMatrix mx_in_ = mx_in;
    ...
    OldMatrix&
    OldMatrix::pow(unsigned int n)
    {
      if (m_nrows != m_ncols)
        throw Error("OldMatrix is not square!");

      OldMatrix power(m_nrows);
    ...
    void
    OldMatrix::toFile(const char* path)
    {
    ...
    void
    OldMatrix::fromFile(const char* path)
    {
    ...
    OldMatrix
    OldMatrix::row(size_t i) const
    {
      return get(i, i, 0, m_ncols - 1);
    }

    OldMatrix
    OldMatrix::column(size_t j) const
    {
      return get(0, m_nrows - 1, j, j);
    }

    void
    OldMatrix::swapColumns(size_t i, size_t j)
    {
      ...
      OldMatrix tmp = this->column(i);
      this->put(0, i, this->column(j));
      this->put(0, j, tmp);
    }

    void
    OldMatrix::swapRows(size_t i, size_t j)
    {
      ...
      OldMatrix tmp = this->row(i);
      this->put(i, 0, this->row(j));
      this->put(j, 0, tmp);
    }

    void
    OldMatrix::set_precision(double p)
    {
      if (p > 0)
        precision = p;
    }
    ...
    double
    OldMatrix::get_precision(void)
    {
      return precision;
    }

    void
    OldMatrix::resize(size_t r, size_t c)
    {
      erase();
    ...
    void
    OldMatrix::resizeAndKeep(size_t r, size_t c)
    {
    ...
    void
    OldMatrix::resizeAndFill(size_t r, size_t c, double value)
    {
      resize(r, c);
      fill(value);
    }
    ...
    void
    OldMatrix::resize(const OldMatrix& m)
    {
      resize(m.m_nrows, m.m_ncols);
    }

    double&
    OldMatrix::operator()(size_t i, size_t j)
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    double
    OldMatrix::operator()(size_t i, size_t j) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    double
    OldMatrix::operator()(size_t i) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    double
    OldMatrix::element(size_t i, size_t j) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    bool
    OldMatrix::operator==(const OldMatrix& m) const
    {
      if (m_size != m.m_size)
        return false;
    ...
    OldMatrix&
    OldMatrix::operator=(const OldMatrix& m)
    {
    ...
    OldMatrix
    OldMatrix::operator-(void) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");

      OldMatrix tmp = -1 * *this;
      return tmp;
    }

    OldMatrix&
    OldMatrix::operator*=(double x)
    {
      split();
    ...
    void
    OldMatrix::put(size_t i, size_t j, const OldMatrix& a)
    {
      if (a.isEmpty())
        return;
    ...
    OldMatrix
    OldMatrix::mminor(size_t i, size_t j) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    void
    OldMatrix::lu(OldMatrix& L, OldMatrix& U) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
      ...
      OldMatrix A = *this;
      OldMatrix Lf(m_nrows), dI(m_nrows), P(m_nrows);
      dI = dI + dI;
    ...
    unsigned int
    OldMatrix::lup(OldMatrix& L, OldMatrix& U, OldMatrix& P) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
      ...
      OldMatrix A = *this;
      OldMatrix Lf(m_nrows), dI(m_nrows), Per(m_nrows);
      dI = dI + dI;

      for (size_t i = 0; i < m_nrows - 1; i++)
      {
        if (OldMatrix::precision >= std::fabs(A(i, i)))
        {
          bool p = 0;
          for (size_t k = i + 1; k < m_nrows; k++)
            if (OldMatrix::precision >= std::fabs(A(k, i)))
            {
              A.swapRows(i, k);
              Per.swapRows(i, k);
              p = true;
          ...
          if (!p)
            throw Error("OldMatrix is not invertible!");
          else
            permutations++;
        }

        OldMatrix Lt(m_nrows); // gaussian matrix
        for (size_t j = i + 1; j < m_nrows; j++)
          Lt(j, i) = -A(j, i) / A(i, i);
    ...
    OldMatrix::det(void) const
    {
      ...
      {
        OldMatrix L(m_nrows), U(m_nrows), P(m_nrows);
    ...
    bool
    OldMatrix::Sylvester(void) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    OldMatrix::toDCM(void) const
    {
      ...
      if (m_nrows == 3 && m_ncols == 1)
      {
        ...
        return transpose(OldMatrix(rx, 3, 3) * OldMatrix(ry, 3, 3) * OldMatrix(rz, 3, 3));
      }
    ...
    OldMatrix
    OldMatrix::toQuaternion(void) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    OldMatrix
    OldMatrix::toEulerAngles(void) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    OldMatrix
    OldMatrix::expmts(double tol) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    double
    OldMatrix::norm_p(double p) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");
    ...
    OldMatrix
    OldMatrix::multiply(const OldMatrix& m2)
    {
      if (isEmpty() || m2.isEmpty())
        throw Error("Trying to access an empty matrix!");

      if (m_ncols != m2.m_nrows)
        throw OldMatrix::Error("Incompatible dimensions!");

      OldMatrix s(m_nrows, m2.m_ncols);
    ...
    operator+(const OldMatrix& m1, const OldMatrix& m2)
    {
      if (m1.isEmpty() || m2.isEmpty())
        throw OldMatrix::Error("Trying to access an empty matrix!");

      if (m1.m_nrows != m2.m_nrows || m1.m_ncols != m2.m_ncols)
        throw OldMatrix::Error("Incompatible dimensions!");

      OldMatrix s(m1.m_nrows, m1.m_ncols);
    ...
    operator*(const OldMatrix& m1, const OldMatrix& m2)
    {
      ...
      OldMatrix s(m1.m_nrows, m2.m_ncols);
    ...
    operator/(const OldMatrix& a, const OldMatrix& b)
    {
      ...
      OldMatrix s(a.m_nrows, a.m_ncols);
    ...
    operator>>(std::istream& is, OldMatrix& a)
    {
      ...
      while (!is.eof())
      {
        ...
        if (b == ',')
        {
          ...
          if (elements.size())
          {
            a.vertCat(OldMatrix(&elements[0], 1, elements.size()));
            elements.clear();
          }
        }
    ...
    bool
    OldMatrix::isInvertible(void) const
    {
      if (isEmpty())
        throw Error("Trying to access an empty matrix!");

      if (m_nrows != m_ncols)
        throw OldMatrix::Error("Inversion of a nonsquare OldMatrix!");
      ...
      int rv = upper_triangular_pp(M, n, n + n, OldMatrix::precision);
    ...
    inverse_pp(const OldMatrix& a)
    {
      ...
      int result = Math::OldMatrix::upper_triangular_pp(M, n, n + n, OldMatrix::precision);

      OldMatrix s(n, n);

      if (result)  // singular OldMatrix
      {
        std::free(M);
        throw OldMatrix::Error("Inversion error!");
      }
    ...
    OldMatrix
    inverse_pp(const OldMatrix& a, const OldMatrix& b)
    {
      if (a.isEmpty() || b.isEmpty())
        throw OldMatrix::Error("Trying to access an empty matrix!");
      ...
      int result = Math::OldMatrix::upper_triangular_pp(M, n, n + m, OldMatrix::precision);
    ...
    inverse(const OldMatrix& a)
    {
      ...
      int result = Math::OldMatrix::upper_triangular_tp(M, index, n, n + n, OldMatrix::precision);
    ...
    inverse(const OldMatrix& a, const OldMatrix& b)
    {
      ...
      int result = Math::OldMatrix::upper_triangular_tp(M, index, n, n + m, OldMatrix::precision);
    ...
    //     throw OldMatrix::Error("OldMatrix must be 3x1 or 1x3 to create a skew symmetrical!");
    ...
    inverse_lup(const OldMatrix& a)
    {
      ...
      for (int k = 0; k < n; k++)
      {
        ...
        for (int j = n - 1; j >= 0; j--)
        {
          ...
          if (OldMatrix::precision >= std::fabs(u[j * n + j]))
            throw OldMatrix::Error("OldMatrix is not invertible!");
          else
            m[j * n + k] /= u[j * n + j];
        }
      }
    ...
    int
    OldMatrix::upper_triangular_pp(double* M, int n, int m, double tolerance)
    {
      for (int i = 0; i < n; i++)
      {
        int ii = i;
    ...
    int
    OldMatrix::upper_triangular_tp(double* M, int* index, int n, int m, double tolerance)
    {
      for (int i = 0; i < n; i++)
      {
        int ii = i;
    ...
    double
    OldMatrix::dot(const OldMatrix& a, const OldMatrix& b)
    {
    ...
    OldMatrix
    OldMatrix::cross(const OldMatrix& a, const OldMatrix& b)
    {
      if (a.isEmpty() || a.isEmpty())
        throw OldMatrix::Error("Trying to access an empty matrix!");
    ...
    void
    OldMatrix::readFromLines(const std::vector<std::string>& clines)
    {
      if (isEmpty())
    ...
    void
    OldMatrix::readFromConfig(DUNE::Parsers::Config& cfg, const std::string& section, const std::string& param)
    {
