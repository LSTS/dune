      ...
      namespace Models
      {
        namespace RPM
        {
          ...
          public:
            ...
            uint16_t
            getPWM(const float& voltage, const int16_t& rpm) override
            {
              uint16_t pwm = 0;
              if (voltage < 11.0f)
                pwm = Math::piecewiseLI(c_pwm, c_rpm_10v, rpm);
              else if (voltage >= 11.0f && voltage < 13.0f)
                pwm = Math::piecewiseLI(c_pwm, c_rpm_12v, rpm);
              else if (voltage >= 13.0f && voltage < 15.0f)
                pwm = Math::piecewiseLI(c_pwm, c_rpm_14v, rpm);
              else if (voltage >= 15.0f && voltage < 17.0f)
                pwm = Math::piecewiseLI(c_pwm, c_rpm_16v, rpm);
              else if (voltage >= 17.0f && voltage < 19.0f)
                pwm = Math::piecewiseLI(c_pwm, c_rpm_18v, rpm);
              else if (voltage >= 19.0f)
                pwm = Math::piecewiseLI(c_pwm, c_rpm_20v, rpm);
