...
#ifndef DUNE_UTILS_ACTUATORS_T200_HPP_INCLUDED_
#define DUNE_UTILS_ACTUATORS_T200_HPP_INCLUDED_
...
namespace DUNE
{
  namespace Utils
  {
    namespace Actuators
    {
      ...
      class T200 : public Actuators::Model
      {
      public:
        //! Constructor.
        T200(void) = default;
        ...
        int16_t
        getRPM(const float& voltage, const int16_t& duty_cycle) override
        {
          int16_t rpm = 0;
          if (voltage < 11.0f)
            rpm = Math::piecewiseLI(c_rpm_10v, c_pwm, duty_cycle);
          else if (voltage >= 11.0f && voltage < 13.0f)
            rpm = Math::piecewiseLI(c_rpm_12v, c_pwm, duty_cycle);
          else if (voltage >= 13.0f && voltage < 15.0f)
            rpm = Math::piecewiseLI(c_rpm_14v, c_pwm, duty_cycle);
          else if (voltage >= 15.0f && voltage < 17.0f)
            rpm = Math::piecewiseLI(c_rpm_16v, c_pwm, duty_cycle);
          else if (voltage >= 17.0f && voltage < 19.0f)
            rpm = Math::piecewiseLI(c_rpm_18v, c_pwm, duty_cycle);
          else if (voltage >= 19.0f)
            rpm = Math::piecewiseLI(c_rpm_20v, c_pwm, duty_cycle);
