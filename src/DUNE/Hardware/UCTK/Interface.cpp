...
namespace DUNE
{
  namespace Hardware
  {
    namespace UCTK
    {
      ...
      FirmwareInfo
      Interface::getFirmwareInfo(double timeout)
      {
        Time::Counter<double> timer(timeout);
        FirmwareInfo info;
        getFirmwareName(info, timer.getRemaining());
        getFirmwareVersion(info, timer.getRemaining());
        return info;
      }
      ...
      void
      Interface::getFirmwareName(FirmwareInfo& info, double timeout)
      {
        UCTK::Frame frame;
        frame.setId(PKT_ID_NAME);
        frame.setPayloadSize(0);

        if (!sendFrame(frame, timeout))
      ...
      void
      Interface::getFirmwareVersion(FirmwareInfo& info, double timeout)
      {
        UCTK::Frame frame;
