...
namespace DUNE
{
  namespace Parsers
  {
    ...
    Config::parseFile(const char* fname)
    {
      ...
      while (std::fscanf(fd, " %1023[^\n] ", line) == 1)
      {
        ...
        else if (getOptionAndValue(line, option, arg))
        {
          ...
          Table::iterator sitr = m_data.find(isec);
          if (sitr == m_data.end())
    ...
    Config::sections()
    {
      ...
      for (Table::iterator itr = m_data.begin(); itr != m_data.end(); ++itr)
    ...
    {
      Concurrency::ScopedRWLock lock(m_lock);
      Table::const_iterator sitr = m_data.find(section);
    ...
    Config::writeToStream(std::ostream& os)
    {
      ...
      Config::Table::const_iterator sects;
