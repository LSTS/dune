...
// #include "../../../Control/ASV/Autopilot/FilterEstimator.hpp"
        ...
        struct Task : public DUNE::Tasks::Task
        {
          ...
          consume(const IMC::EulerAngles *msg)
          {
            if (std::strcmp(resolveEntity(msg->getSourceEntity()).c_str(), "HemisphereGPS") == 0)
            {
              ...
              trace("ESTIMATOR - EULER ANGLES FROM HemisphereGPS: theta = %f phi = %f", Angles::degrees(m_euler.theta), Angles::degrees(m_euler.phi));
            }
          }
          ...
          consume(const IMC::Displacement *msg)
          {
            ...
            m_cg_heave.z = m_gps_heave + r_x * std::sin(m_euler.theta) - r_z * std::cos(m_euler.theta) * std::cos(m_euler.phi) - r_y * std::cos(m_euler.theta) * std::sin(m_euler.phi);
          ...
          estimation(void)
          {
            ...
            m_zeta_2_dot = -2 * m_args.lpf_est_cutoff * m_zeta_2 - std::pow(m_args.lpf_est_cutoff, 2) * m_zeta_1 + std::pow(m_args.lpf_est_cutoff, 2) * m_cg_heave.z;
