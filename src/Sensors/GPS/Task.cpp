    ...
    struct Task: public Hardware::BasicDeviceDriver
    {
      ...
      interpretSentence(std::vector<std::string>& parts)
      {
        ...
        if (parts[0] == m_args.stn_order.back())
        {
          ...
          std::sprintf(m_bufer_entity, "hdop: %.2f , Sat: %d", m_fix.hdop, m_fix.satellites);
          if (m_fix.validity & IMC::GpsFix::GFV_VALID_POS)
            setStateMsg(CODE_ACTIVE, Utils::String::str(DTR(m_bufer_entity)));
          else
            setStateMsg(CODE_WAIT_GPS_FIX, Utils::String::str(DTR(m_bufer_entity)));
        }
      }
