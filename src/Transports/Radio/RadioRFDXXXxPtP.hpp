...
#ifndef DUNE_TRANSPORTS_RADIO_RADIORFDXXXXPTP_HPP_INCLUDED_
#define DUNE_TRANSPORTS_RADIO_RADIORFDXXXXPTP_HPP_INCLUDED_
...
namespace Transports
{
  namespace Radio
  {
    ...
    //! @author João Teixeira.
    class RadioRFDXXXxPtP: public RadioDriver
    {
      public:
    //! Entity states.
        enum At_cmd
        ...
        DeviceVerifyConf()
        {
          ...
          sendCommandAndWait(commandCreate(REBOOT,data),6);
          Delay::wait(3);
          sendCommandAndWait(commandCreate(ENABLE_CMD,data),1);
        ...
        commandCreate(At_cmd cmd_type, std::string& data)
        {
          ...
          {
            command = AT_RFDPTP[cmd_type];
          }
          else
          {
            if(data.size()>0)
            {
              command =  (AT_RFDPTP[cmd_type]+data + "\r\n");
            }
            else
            {
              std::string cmd = AT_RFDPTP[cmd_type];
              command = (cmd + "\r\n");
            }
          }
       ...
       void
        ...
        process(const std::string msg)
        {
          ...
          try
          {
            ...
            if(cmd_mode)
            {
                if(String::startsWith(last_command,AT_RFDPTP[RADIO_VERSION]))
                {
                  if(String::startsWith(msg,"RFD SiK 2.60"))
                  {
                ...
                if(String::startsWith(last_command,AT_RFDPTP[BOARD_TYPE]))
                {
                  if(String::startsWith(msg,"131"))
                  {
                ...
                if(String::startsWith(last_command,AT_RFDPTP[BOARD_FREQUENCY]))
                 {
                   if(String::startsWith(msg,"0X0086"))
                   {
                ...
                if(String::startsWith(last_command,AT_RFDPTP[BOARD_VERSION]))
                 {
                   if(String::startsWith(msg,"255"))
                   {
                ...
                if(String::startsWith(last_command,AT_RFDPTP[SETTABLE_EEPROM_PARAMETERS]))
                  {
                ...
                if(String::startsWith(last_command,AT_RFDPTP[TDM_TIMING]))
                 {
...
//  S6:MAVLINK=1\r
//  S7:OPPRESEND=0\r
//  S8:MIN_FREQ=868000\r
//  S9:MAX_FREQ=870000\r
//  S10:NUM_CHANNELS=10\r
//  S11:DUTY_CYCLE=60\r
//  S12:LBT_RSSI=0\r
//  S13:RTSCTS=0\r
//  S14:MAX_WINDOW=20\r
//  S15:ENCRYPTION_LEVEL=0\r
//  S16:GPI1_1R/CIN=0\r
//  S17:GPO1_1R/COUT=0\r
//  S18:ANT_MODE=0\r
//  S19:PKT_DROP_RSSI=0\r
//  R0:TARGET_RSSI=255\r
//  R1:HYSTERESIS_RSSI=50\r
