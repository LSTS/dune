...
{
    dccl.set_parameter(encodeParameterEnum(imc.parameter));
...
{
    imc.parameter = decodeParameterEnum(dccl.parameter());
...
void encodeLoiter(const DUNE::IMC::Loiter& imc, IMC_DCCL::Loiter& dccl)
{
    ...
    dccl.set_type(encodeLoiterTypeEnum(imc.type));
    ...
    dccl.set_direction(encodeDirectionEnum(imc.direction));
...
void decodeLoiter(const IMC_DCCL::Loiter& dccl, DUNE::IMC::Loiter& imc)
{
    ...
    imc.type = decodeLoiterTypeEnum(dccl.type());
    ...
    imc.direction = decodeDirectionEnum(dccl.direction());
...
{
    dccl.set_type(encodeTypeEnum(imc.type));

    dccl.set_op(encodeOperationEnum(imc.op));
...
{
    imc.type = decodeTypeEnum(dccl.type());

    imc.op = decodeOperationEnum(dccl.op());
