  ...
  // Extract  to  DataSeatrac data structure
   ...
   { 
    ...
    switch(message_type)
    {
     case CID_STATUS:
     {
      ...
      if (data_Beacon.cid_status_msg.outputflags_list[0]) //ENVIRONMENT
      {
       ...
       std::memcpy(&data_Beacon.cid_status_msg.environment_temperature , msg_raw + ind + 2 , 2);
     ...
     break;
      ...
      std::memcpy(&data_Beacon.cid_nav_beacon_pos_update_msg.position_easting  , msg_raw + ind + 1 , 2);
      std::memcpy(&data_Beacon.cid_nav_beacon_pos_update_msg.position_northing , msg_raw + ind + 3 , 2);
      std::memcpy(&data_Beacon.cid_nav_beacon_pos_update_msg.position_depth    , msg_raw + ind + 5 , 2);
      ...
      std::memcpy(&data_Beacon.cid_nav_ref_pos_update_msg.position_latitude  , msg_raw + ind + 1 , 4);
      std::memcpy(&data_Beacon.cid_nav_ref_pos_update_msg.position_longitude , msg_raw + ind + 5 , 4);
    ...
    commandCreateSeatrac(CommandID cid_type , DataSeatrac& data_Beacon )
    {
     ...
     {
      case CID_PING_SEND:
      message_build+= String::str("%02X%02X",((uint8_t)data_Beacon.cid_ping_send_msg.dest_id ), ((uint8_t) data_Beacon.cid_ping_send_msg.msg_type));
      break;
      
      case CID_DAT_SEND:  
      message_build+= String::str("%02X%02X%02X",((uint8_t)data_Beacon.cid_dat_send_msg.dest_id ),((uint8_t)data_Beacon.cid_dat_send_msg.msg_type),((uint8_t)data_Beacon.cid_dat_send_msg.packet_len));
      ...
      case  CID_NAV_BEACON_POS_SEND:
      {
       ...
       std::memcpy( &msg_temp[3]  , &data_Beacon.cid_nav_beacon_pos_send_msg.position_northing, 2);
      ...
      case CID_NAV_QUERY_SEND:
      {
       message_build+= String::str("%02X%02X",((uint8_t)data_Beacon.cid_nav_query_send_msg.dest_id ), ((uint8_t) data_Beacon.cid_nav_query_send_msg.query_flags));
      }
      break;
