...
namespace UserInterfaces
{
  namespace Webpage
  {
    ...
    static constexpr unsigned c_max_request_size = 2048;
    ...
    RequestHandler::sendHeader(TCPSocket* sock, const char* status_line, int64_t length, HeaderFieldsMap* hdr_fields)
    {
      ...
      {
        DUNE_WRN("RequestHandler", e.what());
      }
    }

    void
    RequestHandler::sendResponse(TCPSocket* sock, std::string status)
    {
      try
      {
    ...
    RequestHandler::sendResponse100(TCPSocket* sock)
    {
      try
      {
        ...
        sendHeader(sock, STATUS_LINE_100, 8, &hdr);
        sendResponse(sock, "Continue");
      }
      catch(const std::exception& e)
      {
        DUNE_WRN("RequestHandler", e.what());
    ...
    RequestHandler::sendResponse200(TCPSocket* sock)
    {
      try
      {
        RequestHandler::HeaderFieldsMap hdr;
        hdr["Content-Type"] = "application/json";
        sendHeader(sock, STATUS_LINE_200, 2, &hdr);
    ...
    RequestHandler::sendResponse201(TCPSocket* sock)
    {
      try
      {
        RequestHandler::HeaderFieldsMap hdr;
        hdr["Content-Type"] = "application/json";
        sendHeader(sock, STATUS_LINE_201, 7, &hdr);
        sendResponse(sock, "Created");
      }
      catch(const std::exception& e)
      {
        DUNE_WRN("RequestHandler", e.what());
    ...
    RequestHandler::sendResponse403(TCPSocket* sock)
    {
      try
      {
        RequestHandler::HeaderFieldsMap hdr;
        hdr["Content-Type"] = "application/json";
        sendHeader(sock, STATUS_LINE_403, 9, &hdr);
        sendResponse(sock, "Forbidden");
      }
      catch(const std::exception& e)
      {
        DUNE_WRN("RequestHandler", e.what());
    ...
    RequestHandler::sendResponse404(TCPSocket* sock, const std::string& message)
    {
      try
      {
        RequestHandler::HeaderFieldsMap hdr;
        hdr["Content-Type"] = "application/json";
        sendHeader(sock, STATUS_LINE_404, message.size(), &hdr);
        sendResponse(sock, message);
      }
      catch(const std::exception& e)
      {
        DUNE_WRN("RequestHandler", e.what());
    ...
    RequestHandler::sendResponse416(TCPSocket* sock)
    {
      try
      {
        RequestHandler::HeaderFieldsMap hdr;
        hdr["Content-Type"] = "application/json";
        sendHeader(sock, STATUS_LINE_416, 31, &hdr);
        sendResponse(sock, "Requested Range Not Satisfiable");
      }
      catch(const std::exception& e)
      {
        DUNE_WRN("RequestHandler", e.what());
    ...
    RequestHandler::sendResponse500(TCPSocket* sock)
    {
      try
      {
        RequestHandler::HeaderFieldsMap hdr;
        hdr["Content-Type"] = "application/json";
        sendHeader(sock, STATUS_LINE_500, 21, &hdr);
        sendResponse(sock, "Internal Server Error");
      }
      catch(const std::exception& e)
      {
        DUNE_WRN("RequestHandler", e.what());
    ...
    RequestHandler::sendResponse503(TCPSocket* sock)
    {
      try
      {
        RequestHandler::HeaderFieldsMap hdr;
        hdr["Content-Type"] = "application/json";
        sendHeader(sock, STATUS_LINE_503, 19, &hdr);
        sendResponse(sock, "Service unavailable");
      }
      catch(const std::exception& e)
      {
        DUNE_WRN("RequestHandler", e.what());
    ...
    RequestHandler::handleRequest(TCPSocket* sock)
    {
      ...
      if (std::sscanf(hdr, "%s %s %*s", mtd, uri) == 2)
      {
        ...
        {
          sockHasToBeDeleted = handleGET(sock, headers, uri_clean);
        }
        else if (std::strcmp(mtd, "POST") == 0)
        {
          sockHasToBeDeleted = handlePOST(sock, headers, uri_clean);
        }
        else if (std::strcmp(mtd, "PUT") == 0)
        {
          sockHasToBeDeleted = handlePUT(sock, headers, uri_clean);
        }
      }

      delete[] hdr;
      return sockHasToBeDeleted;
    }
  }
}
